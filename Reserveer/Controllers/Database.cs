        ...
        {
            {
                try
                {
                    List<string[]> GroupRooms = new List<string[]>(); //List of string []'s to store results
                    using (MySqlConnection connMysql = new MySqlConnection(connString))//setup connection
                    {
                        using (MySqlCommand cmdd = connMysql.CreateCommand())
                        {
                            int groupID = 1;
                            cmdd.CommandText = "SELECT rooms.room_id, rooms.room_name, rooms.available FROM `rooms` where " + groupID + " = rooms.group_id"; //Sql query to be executed
                            cmdd.CommandType = System.Data.CommandType.Text;
                            cmdd.Connection = connMysql;
        ...
        {
            {
                try
                {
                    List<string[]> GroupUser = new List<string[]>(); //List of string []'s to store results
                    using (MySqlConnection connMysql = new MySqlConnection(connString)) // setup connection
                    {
                        using (MySqlCommand cmdd = connMysql.CreateCommand())
                        {
                            int groupID = 1;
                            cmdd.CommandText = "SELECT user.user_id, user.user_name, user.user_mail, user.active, user.group_id FROM `user` WHERE user.group_id = " + groupID + " AND user.active = 0"; //Sql query to be executed
                            cmdd.CommandType = System.Data.CommandType.Text;
                            cmdd.Connection = connMysql;
